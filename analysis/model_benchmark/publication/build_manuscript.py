#!/usr/bin/env python3
"""Build the Markdown manuscript from the audited publication/data aggregates.

Run build_analysis.py first. This script is offline and makes no model calls.
The narrative is part of this source; numerical tables are generated from CSV.
"""
...
def num(x, digits=2):
    return "NA" if x in (None, "") else f"{float(x):.{digits}f}"
...
def figure(n, name, alt, caption):
    return f"![{alt}](publication/figures/{name})\n\n**Figure {n}.** {caption}"


def main():
    summary=read("condition_summary.csv")
    sm={(r["model"],r["condition"]):r for r in summary}
    models=list(dict.fromkeys(r["model"] for r in summary))
    labels={r["model"]:r["label"] for r in summary}
    conds=["thinking_off","effort_low","effort_high"]
    cname=dict(zip(conds+["control_no_clues"],["Off","Low","High","No clues / high"]))
    audit=json.loads((DATA/"audit.json").read_text())
    items=read("item_inventory.csv")
    item_ids=[r["instance_id"] for r in items]
    human=read("human_item_context.csv")
    mitems=read("model_item_summary.csv")
    mis={(r["model"],r["condition"],r["instance_id"]):r for r in mitems}
    trials=read("model_trials.csv")
    controls=read("control_response_distribution.csv")
    contrasts=read("effort_contrasts.csv")
    errors=read("incorrect_responses.csv")

    acc=table(["Model","Off: correct/N (%)","Off: Wilson 95% reference interval","Low: correct/N (%)","High: correct/N (%)"],
        [[labels[m],f'{sm[m,conds[0]]["correct"]}/30 ({num(sm[m,conds[0]]["accuracy_pct"],1)})',
          f'{num(sm[m,conds[0]]["wilson_low_pct"],1)}–{num(sm[m,conds[0]]["wilson_high_pct"],1)}%',
          "30/30 (100.0)","30/30 (100.0)"] for m in models])
    offitem=table(["Model"]+item_ids+ ["All items"],
        [[labels[m]]+[f'{mis[m,conds[0],i]["correct"]}/5' for i in item_ids]+[f'{sm[m,conds[0]]["correct"]}/30'] for m in models])
    token=table(["Model","Effort","Output median [Q1, Q3]","Reasoning median","Non-reasoning output median"],
        [[r["label"],cname[r["condition"]],f'{num(r["median_output_tokens"],1)} [{num(r["q1_output_tokens"],1)}, {num(r["q3_output_tokens"],1)}]',
          num(r["median_reasoning_tokens"],1),num(r["median_nonreasoning_output_tokens"],1)] for r in summary if r["condition"] in conds])
    latency=table(["Model","Effort","Wall median [Q1, Q3], s","Claude API median, s","Output tokens / correct","Wall seconds / correct"],
        [[r["label"],cname[r["condition"]],f'{num(r["median_wall_s"])} [{num(r["q1_wall_s"])}, {num(r["q3_wall_s"])}]',num(r["median_api_s"]),num(r["output_tokens_per_correct"]),num(r["wall_seconds_per_correct"])] for r in summary if r["condition"] in conds])
    contrast=table(["Model","Off → low accuracy, pp","Low → high accuracy, pp","High/low median output","High/low median wall"],
        [[labels[r["model"]],num(r["off_to_low_accuracy_pp"],1),num(r["low_to_high_accuracy_pp"],1),num(r["high_to_low_median_output_ratio"])+"×",num(r["high_to_low_median_wall_ratio"])+"×"] for r in contrasts])
    control_table=table(["Model","Correct/N (%)","Most frequent response (count)","Parse failures","Median output / reasoning tokens","Median wall, s"],
        [[labels[m],f'{sm[m,"control_no_clues"]["correct"]}/30 ({num(sm[m,"control_no_clues"]["accuracy_pct"],2)})',
          "{} ({})".format(*max([(r["submitted"],int(r["count"])) for r in controls if r["model"]==m],key=lambda x:x[1])),
          sm[m,"control_no_clues"]["parse_failures"],f'{num(sm[m,"control_no_clues"]["median_output_tokens"],1)} / {num(sm[m,"control_no_clues"]["median_reasoning_tokens"],1)}',num(sm[m,"control_no_clues"]["median_wall_s"])] for m in models])
    visible=table(["Model","Visible characters: off / low / high","Median distinct cells: off / low / high","Enumeration-flag %: off / low / high"],
        [[labels[m]," / ".join(num(sm[m,c]["median_visible_characters"],1) for c in conds),
          " / ".join(num(sm[m,c]["median_cells_named"],1) for c in conds),
          " / ".join(num(sm[m,c]["enumeration_pct"],1) for c in conds)] for m in models])
    error_counts=Counter((r["instance_id"],r["submitted"],r["expected"],r["violated_clue_numbers"]) for r in errors)
    error_table=table(["Item","Incorrect response","Correct target","Errors / 15 GPT off sessions","Only violated clue"],
        [[iid,sub,expected,f"{n}/15",f"#{clue}: "+("diagonal exclusion" if iid=="I0014" else "middle-four exclusion")]
         for (iid,sub,expected,clue),n in sorted(error_counts.items())])
    human_table=table(["Item","Correct/N (%)","Task time median [Q1, Q3], s","Mean attempts","Orientation / execution medians, s"],
        [[r["instance_id"],f'{r["correct"]}/18 ({num(r["accuracy_pct"],1)})',
          f'{num(r["median_solve_s"])} [{num(r["q1_solve_s"])}, {num(r["q3_solve_s"])}]',num(r["mean_attempts"],3),f'{num(r["median_orientation_s"])} / {num(r["median_execution_s"])}'] for r in human])
    counters=read("claude_token_counter_sensitivity.csv")
    counter_table=table(["Haiku condition","Primary median","Model-usage median","Difference range per session","Disagreeing/N"],
        [[cname[r["condition"]],num(r["median_primary_output_tokens"],1),num(r["median_model_usage_output_tokens"],1),f'{r["min_delta"]}–{r["max_delta"]}',f'{r["rows_disagreeing"]}/30'] for r in counters if r["model"]=="haiku"])
    strict_rows=[]
    for m in models:
        for cond in conds:
            rs=[r for r in trials if r["model_alias"]==m and r["condition"]==cond]
            missing=[r for r in rs if r["has_answer_marker"]=="False"]
            if missing:
                hits=sum(r["correct"]=="True" for r in rs)
                strict_hits=sum(r["correct"]=="True" and r["has_answer_marker"]=="True" for r in rs)
                strict_rows.append([labels[m],cname[cond],f"{hits}/30 ({hits/30*100:.2f}%)",f"{strict_hits}/30 ({strict_hits/30*100:.2f}%)",len(missing)])
    strict_table=table(["Model","Condition","Original parser","Marker-required parser","Correct bare-cell responses"],strict_rows)
    loo=read("leave_one_item_out.csv")
    loo_table=table(["Model","Full off accuracy","Range after omitting one item (25 sessions)"],
        [[labels[m],num(sm[m,conds[0]]["accuracy_pct"],1)+"%",
          f'{min(float(r["accuracy_pct"]) for r in loo if r["model"]==m):.1f}–{max(float(r["accuracy_pct"]) for r in loo if r["model"]==m):.1f}%'] for m in models])
    resource_totals=table(["Arm / subset","Completed N","Correct","Total output tokens","Total reasoning tokens","Sum of session wall times, s"],
        [[vendor+" / "+subset, sum(int(r["n"]) for r in summary if r["model"] in ms and (r["condition"]=="control_no_clues")==ctrl),
          sum(int(r["correct"]) for r in summary if r["model"] in ms and (r["condition"]=="control_no_clues")==ctrl),
          sum(int(r["sum_output_tokens"]) for r in summary if r["model"] in ms and (r["condition"]=="control_no_clues")==ctrl),
          sum(int(r["sum_reasoning_tokens"]) for r in summary if r["model"] in ms and (r["condition"]=="control_no_clues")==ctrl),
          num(sum(float(r["sum_wall_s"]) for r in summary if r["model"] in ms and (r["condition"]=="control_no_clues")==ctrl))]
         for vendor,ms in [("Claude",models[:3]),("GPT",models[3:])] for subset,ctrl in [("clues",False),("controls",True)]])

    manuscript=f'''# Reasoning Effort and Failure Modes in a Text-Based Grid CAPTCHA: A Comparative Benchmark of Claude and GPT Subscription Models
...
| `output/provisional_incomplete/` (removed from the repository on 15 September 2026) | Superseded summaries from incomplete GPT collection | Excluded; not pooled with the completed run |
| `run_benchmark.py` and `analyze_benchmark.py` API design (never run; removed on 15 September 2026) | Separate design with different prompts and up to three attempts | No `output/runs.jsonl` dataset ever existed; no API-arm results or cost estimates treated as observations |
...
GPT completed-session start timestamps run from `{audit['gpt_first_started_at']}` to `{audit['gpt_last_started_at']}`. This is a collection window, not the sum of inference durations. Three quota-related error records were followed by successful completion of their planned session keys. Claude raw records do not retain a comparable collection timestamp or CLI version. The date-like suffix in a model identifier is not a collection date.
...
{figure(1,'01_accuracy_by_effort.png','Accuracy by effort: Claude 100% in all conditions; GPT off 60%, 60%, 80%, and GPT low/high 100%.','Correct first responses and Wilson reference intervals. The uncertainty bars are conditional descriptive references; they do not represent uncertainty over the full population of possible grid puzzles.')}
...
{figure(2,'02_thinking_off_item_accuracy.png','Thinking-off success counts out of five for each of the six models and six items.','Errors are concentrated in the four items containing region exclusions. GPT Luna and Terra each solved both axis-only items in all repetitions; all three GPT configurations failed every off repetition of the diagonal item I0014.')}
...
{figure(3,'03_output_token_distributions.png','Boxplots of output-token counters for six models in off, low and high conditions.','Boxes show the interquartile range, center lines the median, whiskers 1.5 times the interquartile range and points outliers. The horizontal scale is logarithmic. Haiku counters are examined separately in Section 3.8; cross-provider token counts are not standardized compute units.')}
...
{figure(4,'04_visible_response_lengths.png','Visible answer-text lengths differ across models even with the same supplied instructions.','Character counts provide a view independent of vendor tokenizers. They quantify saved answer text, not total internal processing. The logarithmic scale accommodates bare-cell outputs and longer explanations.')}
...
{figure(5,'05_wall_latency_distributions.png','End-to-end CLI wall latency for each model and effort condition.','This comparison uses wall time for both vendors. Claude API time is presented only in Table 13. Differences include CLI overhead and service conditions; Claude was sequential and GPT used three workers. These are not matched measurements of model inference speed.')}
...
{figure(6,'07_low_high_resource_contrast.png','Low and high effort resource medians, with no observed accuracy gain at high effort.','Squares denote low effort and triangles high effort; labels show high/low median ratios. The token panel uses a log scale. Every model scored 30/30 at both settings with the original parser.')}
...
{figure(7,'06_no_clue_response_distribution.png','No-clue answers concentrate heavily on A1, with a different spread for Haiku.','Cells show response counts, including a separate unparsed category. A1 is not a target in the canonical set, so a constant A1 strategy necessarily scores zero despite requiring no clues. The response distribution, not correctness alone, is necessary to interpret this control.')}
...
{figure(8,'08_human_context.png','Human eventual item accuracy and observed task-time medians with interquartile ranges.','Time whiskers show the distribution between participants, not confidence intervals. The two slowest item medians were I0012 and I0014, both about 108 s. Human repetitions, practice, interface operation and fixed order differ from the independent model calls.')}
...
'''
    for i in items:
        manuscript+=f'### {i["instance_id"]} — {i["clue_type"]}; target {i["answer"]}\n\n'
        manuscript+="\n".join(f'{j}. {i[f"clue_{j}"]}' for j in range(1,5))+"\n\n"
    manuscript+='''## Appendix B. Resource Totals and Supplementary Data
...
'''
    inputs=read("gpt_input_usage.csv")
    manuscript+=table(["Model","Condition","N","Median input tokens [min, max]","Total cached input tokens"],
        [[labels[r["model"]],cname[r["condition"]],r["n"],f'{num(r["median_input_tokens"],1)} [{r["min_input_tokens"]}, {r["max_input_tokens"]}]',r["sum_cached_input_tokens"]] for r in inputs])
    manuscript+='''

The supplementary CSVs provide exact values beyond the rounded manuscript tables:

'''
    files=[("condition_summary.csv","All 24 model–condition rows, including distribution statistics and controls"),
           ("gpt_input_usage.csv","Input-token range, median, totals and cache counters for all 12 GPT model–condition groups"),
           ("model_item_summary.csv","All 144 model–condition–item rows, including control items"),
           ("model_trials.csv","720 normalized model responses; no human identifiers or model hidden reasoning text"),
           ("item_inventory.csv","All six items and exact clues; answer-bearing analytical material"),
           ("incorrect_responses.csv","All 30 incorrect clue responses and the violated clue"),
           ("item_type_summary.csv","Axis-only versus region-clue aggregates for each model and effort"),
           ("effort_contrasts.csv","Off-to-low and low-to-high descriptive contrasts"),
           ("control_response_distribution.csv","No-clue response counts by model and selected cell"),
           ("claude_token_counter_sensitivity.csv","Primary versus model-usage output counters"),
           ("leave_one_item_out.csv","Six deterministic omissions per model in thinking off"),
           ("human_item_context.csv","Six aggregate human item rows only")]
    manuscript+=table(["File","Contents"],[[f'[{name}](publication/data/{name})',desc] for name,desc in files])
    manuscript+='''
...
'''
    selected=[(p,h) for p,h in audit["source_sha256"].items() if p.endswith(("cli_arm_runs.jsonl","gpt_cli_arm_runs.jsonl","gpt_cli_arm_runs.manifest.json","gpt_inputs.json","data/instances.json","clean_unique_trials.csv"))]
    manuscript+=table(["Repository-relative source","SHA-256"],[[f'`{p}`',f'`{h}`'] for p,h in selected])
    manuscript+=f'\n\nGPT plan manifest ID: `{audit["gpt_manifest_id"]}`. This is the canonical identity digest defined by the collection manifest; it differs from the SHA-256 of the manifest file bytes in Table C1.\n'
    path=BENCH/"COMPARATIVE_BENCHMARK_REPORT.md"
    path.write_text(manuscript)
    print(f"Wrote {path.name}: {len(manuscript.split()):,} whitespace-separated words, {len(manuscript.splitlines())} lines")
